...
- 🎯 **三种分析模式：投机/短期/价值** — 从单一短期趋势分析扩展为三种独立的投资风格分析：
  - **投机分析**（事件驱动 · 2周-1月展望）：聚焦催化剂、市场情绪、事件日历，评分标准为催化剂强度(30%) + 市场关注度(25%) + 事件时效(25%) + 技术配合(20%)
  - **短期分析**（趋势交易 · 1天-2周）：原有分析模式，行为不变
  - **价值分析**（巴菲特风格 · 3-10年展望）：聚焦估值、护城河、盈利质量，评分标准为安全边际(30%) + 护城河(25%) + 盈利质量(25%) + 分红(10%) + 财务健康(10%)
  - 同一股票可同时运行不同分析模式（去重 key 含 analysis_type）
  - API: `POST /api/v1/analysis/analyze` 新增 `analysis_type` 字段（`short_term`/`speculation`/`value`，默认 `short_term`）
  - API: `GET /api/v1/history` 新增 `analysis_type` 查询参数，不传返回全部类型
- 🏠 **概览仪表盘首页** — 首页改为概览仪表盘，展示三种分析入口卡片 + 最近分析记录（跨类型）
- 📊 **市场能力分层声明** — 价值模式下 HK/US 股票标注"受限分析"，提示基本面数据有限，引导 LLM 侧重定性分析
- 🔍 **搜索服务维度级 fallback** — 每个搜索维度最多尝试 2 个 provider，投机模式支持 `stable_first` 策略（Brave 优先）
- 💾 **数据库安全迁移机制** — 自动补充 `analysis_type` 列与索引，兼容新旧数据库，无需手动迁移
- ⏱️ **价值模式基本面超时分层** — 价值模式 bundle 超时 5s、总预算 12s、缓存 TTL 600s，跳过龙虎榜和板块数据
- 📈 **价值模式 PEG 计算** — Pipeline 自动从 PE_TTM 和 net_profit_yoy 计算 PEG 并注入上下文
- 🛡️ **数据质量守卫** — 投机模式新闻维度全空时注入警告；价值模式基本面数据 3 层降级（完整/部分缺失/严重不足）
- 🏠 **概览页增强** — 入口卡片新增分析模式描述文案 + 跨类型分析汇总统计（近期分析次数、平均情绪分）；历史记录卡片新增操作建议展示
