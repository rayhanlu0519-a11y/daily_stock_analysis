...
const AnalysisPage: React.FC<AnalysisPageProps> = ({ analysisType, title, subtitle }) => {
  ...
  // Sync analysisType from props to store
  useEffect(() => {
    setAnalysisType(analysisType);
  }, [analysisType, setAnalysisType]);

  useEffect(() => {
    document.title = `${title} - DSA`;
  }, [title]);
      ...
      <div className="flex-1 flex flex-col min-w-0 max-w-full lg:max-w-6xl mx-auto w-full">
        <header className="flex min-w-0 flex-shrink-0 items-center overflow-hidden px-3 py-3 md:px-4 md:py-4">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <div className="hidden sm:flex flex-shrink-0 flex-col mr-2">
              <span className="text-sm font-semibold text-foreground leading-tight">{title}</span>
              <span className="text-[10px] text-secondary-text leading-tight">{subtitle}</span>
            </div>
